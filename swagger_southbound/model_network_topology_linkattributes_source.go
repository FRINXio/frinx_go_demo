...
type NetworkTopologyLinkattributesSource struct {
	// Optional[Source node identifier, must be in same topology.] REF:Optional.empty
	SourceNode string `json:"source-node,omitempty"`
	// Optional[Termination point within source node that terminates the link.] REF:Optional.empty
