...
type NetworkTopologyLinkattributesDestination struct {
	// Optional[Termination point within destination node that terminates the link.] REF:Optional.empty
	DestTp string `json:"dest-tp,omitempty"`
	// Optional[Destination node identifier, must be in same topology.] REF:Optional.empty
