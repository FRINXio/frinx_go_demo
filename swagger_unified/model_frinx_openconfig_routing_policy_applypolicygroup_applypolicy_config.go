...
// Optional[Policy configuration data.] REF:Optional.empty
type FrinxOpenconfigRoutingPolicyApplypolicygroupApplypolicyConfig struct {
	...
	// Optional[explicitly set a default policy if no policy definition in the export policy chain is satisfied.] REF:Optional.empty
	DefaultExportPolicy *FrinxOpenconfigRoutingPolicyDefaultPolicyType `json:"default-export-policy,omitempty"`
	...
	// Optional[list of policy names in sequence to be applied on sending a routing update in the current context, e.g., for the current peer group, neighbor, address family, etc.] REF:Optional.empty
