...
// Optional[Configuration parameters relating to ADD_PATHS] REF:Optional.empty
type FrinxOpenconfigBgpBgpcommonstructureneighborgroupaddpathsAddpathsConfig struct {
	// Optional[Enable ability to receive multiple path advertisements for an NLRI from the neighbor or group] REF:Optional.empty
	Receive bool `json:"receive,omitempty"`
	// Optional[A reference to a routing policy which can be used to restrict the prefixes for which add-paths is enabled] REF:Optional.empty
	EligiblePrefixPolicy string `json:"eligible-prefix-policy,omitempty"`
	// Optional[The maximum number of paths to advertise to neighbors for a single NLRI] REF:Optional.empty
	SendMax int32 `json:"send-max,omitempty"`
}
