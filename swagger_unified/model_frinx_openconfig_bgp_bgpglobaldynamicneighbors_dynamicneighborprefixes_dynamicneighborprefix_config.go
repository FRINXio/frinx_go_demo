...
// Optional[Configuration parameters relating to the source prefix for the dynamic BGP neighbor connections.] REF:Optional.empty
type FrinxOpenconfigBgpBgpglobaldynamicneighborsDynamicneighborprefixesDynamicneighborprefixConfig struct {
	// Optional[The peer-group within which the dynamic neighbor will be configured.  The configuration parameters used for the dynamic neighbor are those specified within the referenced peer group.] REF:Optional.empty
	PeerGroup string `json:"peer-group,omitempty"`
	// Optional[The IP prefix within which the source address of the remote BGP speaker must fall to be considered eligible to the dynamically configured.] REF:Optional.empty
