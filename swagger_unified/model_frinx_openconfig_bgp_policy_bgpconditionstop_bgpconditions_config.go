...
type FrinxOpenconfigBgpPolicyBgpconditionstopBgpconditionsConfig struct {
	// Optional[Condition to check if the received MED value is equal to the specified value] REF:Optional.empty
	MedEq int64 `json:"med-eq,omitempty"`
	// Optional[Condition to check if the local pref attribute is equal to the specified value] REF:Optional.empty
	LocalPrefEq int64 `json:"local-pref-eq,omitempty"`
	// Optional[List of next hop addresses to check for in the route update] REF:Optional.empty
	NextHopIn []string `json:"next-hop-in,omitempty"`
	// Optional[Condition to check the route type in the route update] REF:Optional.empty
	RouteType *FrinxOpenconfigBgpPolicyRouteTypeEnumeration `json:"route-type,omitempty"`
	// Optional[List of address families which the NLRI may be within] REF:Optional.empty
	AfiSafiIn []FrinxOpenconfigBgpPolicyAfiSafiInIdentityref `json:"afi-safi-in,omitempty"`
	// Optional[Condition to check if the route origin is equal to the specified value] REF:Optional.empty
	OriginEq *FrinxOpenconfigBgpTypesBgpOriginAttrType `json:"origin-eq,omitempty"`
}
