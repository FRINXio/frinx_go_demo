...
// Optional[This container defines ISIS Overload Bit configuration.] REF:Optional.empty
type FrinxOpenconfigIsisOverloadbitgroupConfig struct {
	// Optional[When set to true, the local IS advertises links with the highest available metric regardless of their configured metric. The metric value is based on the metric style - if wide metrics are utilised the metric is advertised as 16777214, otherwise they are advertised with a value of 63.] REF:Optional.empty
	AdvertiseHighMetric bool `json:"advertise-high-metric,omitempty"`
	// Optional[When set to true, the IS-IS overload bit is set on system boot.] REF:Optional.empty
	SetBitOnBoot bool `json:"set-bit-on-boot,omitempty"`
	// Optional[When set to true, IS-IS overload bit is set.] REF:Optional.empty
