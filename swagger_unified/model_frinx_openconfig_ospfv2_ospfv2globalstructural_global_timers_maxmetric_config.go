...
// Optional[Configuration parameters relating to setting the OSPFv2 maximum metric for a set of advertised entities.] REF:Optional.empty
type FrinxOpenconfigOspfv2Ospfv2globalstructuralGlobalTimersMaxmetricConfig struct {
	// Optional[By default, the maximum metric is advertised for all non-stub interfaces of a device. When identities are specified within this leaf-list, additional entities are also advertised with the maximum metric according to the values within the list.] REF:Optional.empty
	Include []FrinxOpenconfigOspfv2IncludeIdentityref `json:"include,omitempty"`
	// Optional[When this leaf is set to true, all non-stub interfaces of the local system are advertised with the maximum metric, such that the router does not act as a transit system, (similarly to the IS-IS overload functionality).] REF:Optional[RFC3137 - OSPF Stub Router Advertisement]
	Set bool `json:"set,omitempty"`
	// Optional[By default, the maximum metric is only advertised when the max-metric/set leaf is specified as true. In the case that identities are specified within this list, they provide additional triggers (e.g., system boot) that may cause the max-metric to be set. In this case, the system should still honour the timeout specified by the max-metric/timeout leaf, and clear the max-metric advertisements after the expiration of this timer.] REF:Optional.empty
	Trigger []FrinxOpenconfigOspfv2TriggerIdentityref `json:"trigger,omitempty"`
	// Optional[The delay, in seconds, after which the advertisement of entities with the maximum metric should be cleared, and the system reverts to the default, or configured, metrics.] REF:Optional.empty
