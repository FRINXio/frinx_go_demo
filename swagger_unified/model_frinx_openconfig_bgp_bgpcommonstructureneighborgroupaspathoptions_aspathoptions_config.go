...
// Optional[Configuration parameters relating to AS_PATH manipulation for the BGP peer or group] REF:Optional.empty
type FrinxOpenconfigBgpBgpcommonstructureneighborgroupaspathoptionsAspathoptionsConfig struct {
	// Optional[Replace occurrences of the peer's AS in the AS_PATH with the local autonomous system number] REF:Optional.empty
	ReplacePeerAs bool `json:"replace-peer-as,omitempty"`
	// Optional[Specify the number of occurrences of the local BGP speaker's AS that can occur within the AS_PATH before it is rejected.] REF:Optional.empty
	AllowOwnAs int32 `json:"allow-own-as,omitempty"`
}
