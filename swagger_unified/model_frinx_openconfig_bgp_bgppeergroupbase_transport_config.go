...
// Optional[Configuration parameters relating to the transport session(s) used for the BGP neighbor or group] REF:Optional.empty
type FrinxOpenconfigBgpBgppeergroupbaseTransportConfig struct {
	// Optional[Sets the max segment size for BGP TCP sessions.] REF:Optional.empty
	TcpMss int32 `json:"tcp-mss,omitempty"`
	// Optional[Wait for peers to issue requests to open a BGP session, rather than initiating sessions from the local router.] REF:Optional.empty
	PassiveMode bool `json:"passive-mode,omitempty"`
	// Optional[Turns path mtu discovery for BGP TCP sessions on (true) or off (false)] REF:Optional.empty
	MtuDiscovery bool `json:"mtu-discovery,omitempty"`
	// Optional[Set the local IP (either IPv4 or IPv6) address to use for the session when sending BGP update messages.  This may be expressed as either an IP address or reference to the name of an interface.] REF:Optional.empty
