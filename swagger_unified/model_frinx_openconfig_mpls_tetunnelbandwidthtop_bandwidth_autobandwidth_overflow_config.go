...
// Optional[Config information for MPLS overflow bandwidth adjustment] REF:Optional.empty
type FrinxOpenconfigMplsTetunnelbandwidthtopBandwidthAutobandwidthOverflowConfig struct {
	// Optional[number of consecutive overflow sample events needed to trigger an overflow adjustment] REF:Optional.empty
	TriggerEventCount int32 `json:"trigger-event-count,omitempty"`
	// Optional[bandwidth percentage change to trigger an overflow event] REF:Optional.empty
	OverflowThreshold int32 `json:"overflow-threshold,omitempty"`
	// Optional[enables mpls lsp bandwidth overflow adjustment on the lsp] REF:Optional.empty
