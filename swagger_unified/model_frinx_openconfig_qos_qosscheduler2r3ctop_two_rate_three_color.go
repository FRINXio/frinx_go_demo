...
// Optional[Top-level container for data for a 2 rate, 3 color policer.] REF:Optional.empty
type FrinxOpenconfigQosQosscheduler2r3ctopTwoRateThreeColor struct {
	// Optional[Action to be applied to the packets that are scheduled within the PIR of the policer. Packets that receive a token from within the PIR allocation have all the specified actions applied to them] REF:Optional.empty
	ExceedAction *FrinxOpenconfigQosQosscheduler2r3ctopTworatethreecolorExceedAction `json:"exceed-action,omitempty"`
	// Optional[Configuration data for 2 rate, 3 color policers.] REF:Optional.empty
	Config *FrinxOpenconfigQosQosscheduler2r3ctopTworatethreecolorConfig `json:"config,omitempty"`
	// Optional[Action to be applied to the packets that are scheduled above the PIR of the policer. Packets that do not receive a token from either bucket have all specified actions applied to them.] REF:Optional.empty
	ViolateAction *FrinxOpenconfigQosQosscheduler2r3ctopTworatethreecolorViolateAction `json:"violate-action,omitempty"`
	// Optional[Action to be applied to the packets that are scheduled within the CIR of the policer. All packets that receive a token from this bucket have all actions specified applied to them] REF:Optional.empty
	ConformAction *FrinxOpenconfigQosQosscheduler2r3ctopTworatethreecolorConformAction `json:"conform-action,omitempty"`
}
