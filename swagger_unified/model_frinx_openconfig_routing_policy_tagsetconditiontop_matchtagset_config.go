...
// Optional[Configuration data for tag-set conditions] REF:Optional.empty
type FrinxOpenconfigRoutingPolicyTagsetconditiontopMatchtagsetConfig struct {
	// Optional[References a defined tag set] REF:Optional.empty
	TagSet string `json:"tag-set,omitempty"`
	// Optional[Optional parameter that governs the behaviour of the match operation.  This leaf only supports matching on ANY member of the set or inverting the match.  Matching on ALL is not supported] REF:Optional.empty
	MatchSetOptions *FrinxOpenconfigPolicyTypesMatchSetOptionsRestrictedType `json:"match-set-options,omitempty"`
}
