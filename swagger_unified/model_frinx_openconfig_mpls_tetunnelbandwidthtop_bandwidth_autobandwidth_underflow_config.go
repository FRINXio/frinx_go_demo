...
// Optional[Config information for MPLS underflow bandwidth adjustment] REF:Optional.empty
type FrinxOpenconfigMplsTetunnelbandwidthtopBandwidthAutobandwidthUnderflowConfig struct {
	// Optional[number of consecutive underflow sample events needed to trigger an underflow adjustment] REF:Optional.empty
	TriggerEventCount int32 `json:"trigger-event-count,omitempty"`
	// Optional[bandwidth percentage change to trigger and underflow event] REF:Optional.empty
	UnderflowThreshold int32 `json:"underflow-threshold,omitempty"`
	// Optional[enables bandwidth underflow adjustment on the lsp] REF:Optional.empty
