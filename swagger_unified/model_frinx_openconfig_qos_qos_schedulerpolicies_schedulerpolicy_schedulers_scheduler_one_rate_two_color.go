...
// Optional[Top-level container for data related to a 1 rate, 2 color shaper.] REF:Optional.empty
type FrinxOpenconfigQosQosSchedulerpoliciesSchedulerpolicySchedulersSchedulerOneRateTwoColor struct {
	// Optional[Action to be applied to packets that are scheduled above the CIR of the one-rate, two-colour shaper. Packets that do not receive a token from the in-CIR bucket are said to be exceeding, and have all of the specified actions applied.] REF:Optional.empty
	ExceedAction *FrinxOpenconfigQosQosscheduler1r2ctopOneratetwocolorExceedAction `json:"exceed-action,omitempty"`
	// Optional[Configuration data for 1 rate, 2 color shapers] REF:Optional.empty
	Config *FrinxOpenconfigQosQosSchedulerpoliciesSchedulerpolicySchedulersSchedulerOneratetwocolorConfig `json:"config,omitempty"`
	// Optional[Action to be applied to packets that are scheduled within the CIR of the one-rate, two-colour scheduler. Packets that receive a token from the in-CIR bucket are said to be conforming and have all of the specified actions applied.] REF:Optional.empty
	ConformAction *FrinxOpenconfigQosQosscheduler1r2ctopOneratetwocolorConformAction `json:"conform-action,omitempty"`
}
