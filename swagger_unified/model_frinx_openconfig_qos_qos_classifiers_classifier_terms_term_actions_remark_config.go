...
// Optional[Configuration parameters relating to remarking packets.] REF:Optional.empty
type FrinxOpenconfigQosQosClassifiersClassifierTermsTermActionsRemarkConfig struct {
	// Optional.empty REF:Optional.empty
	FrinxQosExtensionsetQosGroup []string `json:"frinx-qos-extension:set-qos-group,omitempty"`
	// Optional[Sets the 3-bit class-of-service value in the Ethernet packet header for 802.1Q VLAN-tagged packets, also known as PCP (priority code point).] REF:Optional[IEEE 802.1Q-2014 - IEEE Standard for Local and metropolitan area networks--Bridges and Bridged Networks]
	SetDot1p int32 `json:"set-dot1p,omitempty"`
	// Optional[Sets the 3-bit traffic class value (also referred to as EXP or CoS) in MPLS packets.] REF:Optional[RFC 3270 - Multi-Protocol Label Switching (MPLS) Support of Differentiated Services]
	SetMplsTc int32 `json:"set-mpls-tc,omitempty"`
	// Optional[List of precedences valid for both ipv4 and ipv6.] REF:Optional.empty
	FrinxQosExtensionsetPrecedences []string `json:"frinx-qos-extension:set-precedences,omitempty"`
	// Optional[Sets the 6-bit DSCP (differentiated services code point) value in the IP packet header.] REF:Optional[RFC 2474 - Definition of the Differentiated Services Field (DS Field) in the IPv4 and IPv6 Headers]
	SetDscp int32 `json:"set-dscp,omitempty"`
}
