...
type FrinxOpenconfigLacpAggregationlacpmemberstopMembers struct {
	// Optional[List of member interfaces and their associated status for a LACP-controlled aggregate interface.  Member list is not configurable here -- each interface indicates items its participation in the LAG.] REF:Optional.empty
	Member []FrinxOpenconfigLacpAggregationlacpmemberstopMembersMember `json:"member,omitempty"`
}
