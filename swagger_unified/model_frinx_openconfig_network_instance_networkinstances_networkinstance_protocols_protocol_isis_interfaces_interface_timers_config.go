...
// Optional[Configuration parameters relating to interface timers for IS-IS] REF:Optional.empty
type FrinxOpenconfigNetworkInstanceNetworkinstancesNetworkinstanceProtocolsProtocolIsisInterfacesInterfaceTimersConfig struct {
	// Optional[The interval interval in milliseconds between the detection of topology change and when the SPF algorithm runs.] REF:Optional.empty
	LspPacingInterval int32 `json:"lsp-pacing-interval,omitempty"`
	// Optional[The number of seconds that the local system waits before retransmitting an unacknowledged LSA.] REF:Optional.empty
	FrinxIsisExtensionretransmissionInterval int64 `json:"frinx-isis-extension:retransmission-interval,omitempty"`
	// Optional[The interval, specified in seconds, at which periodic CSNP packets should be transmitted by the local IS.] REF:Optional.empty
	CsnpInterval int32 `json:"csnp-interval,omitempty"`
}
