...
// Optional[Configuration parameters relating to MPLS auto-bandwidth on the tunnel.] REF:Optional.empty
type FrinxOpenconfigMplsTetunnelbandwidthtopBandwidthAutobandwidthConfig struct {
	// Optional[set the maximum bandwidth in Kbps for an auto-bandwidth LSP] REF:Optional.empty
	MaxBw int32 `json:"max-bw,omitempty"`
	// Optional[time in seconds between adjustments to LSP bandwidth] REF:Optional.empty
	AdjustInterval int64 `json:"adjust-interval,omitempty"`
	// Optional[set the minimum bandwidth in Kbps for an auto-bandwidth LSP] REF:Optional.empty
	MinBw int32 `json:"min-bw,omitempty"`
	// Optional[percentage difference between the LSP's specified bandwidth and its current bandwidth allocation -- if the difference is greater than the specified percentage, auto-bandwidth adjustment is triggered] REF:Optional.empty
	AdjustThreshold int32 `json:"adjust-threshold,omitempty"`
	// Optional[enables mpls auto-bandwidth on the lsp] REF:Optional.empty
