...
// Optional[List of VRRP groups, keyed by virtual router id] REF:Optional.empty
type FrinxOpenconfigIfIpIpvrrptopVrrpVrrpGroup struct {
	// Optional[References the configured virtual router id for this VRRP group] REF:Optional.empty
	VirtualRouterId int32 `json:"virtual-router-id,omitempty"`
	// Optional[Top-level container for VRRP interface tracking] REF:Optional.empty
	InterfaceTracking *FrinxOpenconfigIfIpIpvrrptrackingtopInterfaceTracking `json:"interface-tracking,omitempty"`
	// Optional[Configuration data for the VRRP group] REF:Optional.empty
	Config *FrinxOpenconfigIfIpIpvrrptopVrrpVrrpgroupConfig `json:"config,omitempty"`
}
