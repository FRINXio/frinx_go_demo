...
// Optional[A connection point within a Layer 2 network instance. Each connection-point consists of a set of interfaces only one of which is active at any one time. Other than the specification of whether an interface is local (i.e., exists within this network-instance), or remote, all configuration and state parameters are common] REF:Optional.empty
type FrinxOpenconfigNetworkInstanceNetworkinstancetopNetworkinstancesNetworkinstanceConnectionpointsConnectionPoint struct {
	// Optional[The set of endpoints which are grouped within the connection point] REF:Optional.empty
	Endpoints *FrinxOpenconfigNetworkInstanceNetworkinstancetopNetworkinstancesNetworkinstanceConnectionpointsConnectionpointEndpoints `json:"endpoints,omitempty"`
	// Optional[Configuration parameters relating to a Layer 2 network instance connection point] REF:Optional.empty
	Config *FrinxOpenconfigNetworkInstanceNetworkinstancetopNetworkinstancesNetworkinstanceConnectionpointsConnectionpointConfig `json:"config,omitempty"`
	// Optional[A locally significant reference for the connection-point] REF:Optional.empty
	ConnectionPointId string `json:"connection-point-id,omitempty"`
}
