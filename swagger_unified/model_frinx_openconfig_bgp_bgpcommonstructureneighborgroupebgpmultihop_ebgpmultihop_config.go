...
// Optional[Configuration parameters relating to eBGP multihop for the BGP group] REF:Optional.empty
type FrinxOpenconfigBgpBgpcommonstructureneighborgroupebgpmultihopEbgpmultihopConfig struct {
	// Optional[Time-to-live value to use when packets are sent to the referenced group or neighbors and ebgp-multihop is enabled] REF:Optional.empty
	MultihopTtl int32 `json:"multihop-ttl,omitempty"`
	// Optional[When enabled the referenced group or neighbors are permitted to be indirectly connected - including cases where the TTL can be decremented between the BGP peers] REF:Optional.empty
