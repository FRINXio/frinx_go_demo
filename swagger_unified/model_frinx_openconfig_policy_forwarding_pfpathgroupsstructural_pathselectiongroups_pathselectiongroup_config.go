...
// Optional[Configuration parameters relating to the path selection group.] REF:Optional.empty
type FrinxOpenconfigPolicyForwardingPfpathgroupsstructuralPathselectiongroupsPathselectiongroupConfig struct {
	// Optional[A unique name for the path-selection-group] REF:Optional.empty
	GroupId string `json:"group-id,omitempty"`
	// Optional[A set of MPLS constrained-path LSPs which should be considered for the policy forwarding next-hop. In order to select between the LSPs within the path-selection-group, the system should determine which LSP provides the best path to the next-hop for the routed packet.] REF:Optional.empty
	MplsLsp []string `json:"mpls-lsp,omitempty"`
}
