...
// Optional[Configuration parameters relating to IS-IS actions] REF:Optional.empty
type FrinxOpenconfigIsisPolicyIsisactionsIsisactionsConfig struct {
	// Optional[Set the level that a prefix is to be imported into.] REF:Optional.empty
	SetLevel int32 `json:"set-level,omitempty"`
	// Optional[Set the type of metric that is to be specified when the set metric leaf is specified] REF:Optional.empty
	SetMetricType int32 `json:"set-metric-type,omitempty"`
	// Optional[Set the metric of the IS-IS prefix] REF:Optional.empty
