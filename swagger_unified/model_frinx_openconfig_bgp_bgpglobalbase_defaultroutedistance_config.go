...
// Optional[Configuration parameters relating to the default route distance] REF:Optional.empty
type FrinxOpenconfigBgpBgpglobalbaseDefaultroutedistanceConfig struct {
	// Optional[Administrative distance for routes learned from internal BGP (iBGP).] REF:Optional.empty
	InternalRouteDistance int32 `json:"internal-route-distance,omitempty"`
	// Optional[Administrative distance for routes learned from external BGP (eBGP).] REF:Optional.empty
	ExternalRouteDistance int32 `json:"external-route-distance,omitempty"`
}
