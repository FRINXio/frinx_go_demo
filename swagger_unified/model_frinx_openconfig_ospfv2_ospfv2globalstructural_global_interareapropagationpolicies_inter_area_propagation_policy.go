...
// Optional[A list of connections between pairs of areas - routes are propagated from the source (src) area to the destination (dst) area according to the policy specified] REF:Optional.empty
type FrinxOpenconfigOspfv2Ospfv2globalstructuralGlobalInterareapropagationpoliciesInterAreaPropagationPolicy struct {
	// Optional[Reference to the destination area] REF:Optional.empty
	DstArea string `json:"dst-area,omitempty"`
	// Optional[Reference to the source area] REF:Optional.empty
	...
	Config *FrinxOpenconfigOspfv2Ospfv2globalstructuralGlobalInterareapropagationpoliciesInterareapropagationpolicyConfig `json:"config,omitempty"`
}
