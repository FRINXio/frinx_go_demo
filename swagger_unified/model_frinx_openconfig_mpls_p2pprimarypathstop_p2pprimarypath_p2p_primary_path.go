...
// Optional[List of p2p primary paths for a tunnel] REF:Optional.empty
type FrinxOpenconfigMplsP2pprimarypathstopP2pprimarypathP2pPrimaryPath struct {
	// Optional[Path name] REF:Optional.empty
	Name string `json:"name,omitempty"`
	// Optional[Top-level container for include/exclude constraints for link affinities] REF:Optional.empty
	AdminGroups *FrinxOpenconfigMplsTepathplacementconstraintstopAdminGroups `json:"admin-groups,omitempty"`
	// Optional[The set of candidate secondary paths which may be used for this primary path. When secondary paths are specified in the list the path of the secondary LSP in use must be restricted to those path options referenced. The priority of the secondary paths is specified within the list. Higher priority values are less preferred - that is to say that a path with priority 0 is the most preferred path. In the case that the list is empty, any secondary path option may be utilised when the current primary path is in use.] REF:Optional.empty
	CandidateSecondaryPaths *FrinxOpenconfigMplsP2pprimarypathstopP2pprimarypathP2pprimarypathCandidateSecondaryPaths `json:"candidate-secondary-paths,omitempty"`
	// Optional[Configuration parameters related to paths] REF:Optional.empty
	Config *FrinxOpenconfigMplsP2pprimarypathstopP2pprimarypathP2pprimarypathConfig `json:"config,omitempty"`
}
