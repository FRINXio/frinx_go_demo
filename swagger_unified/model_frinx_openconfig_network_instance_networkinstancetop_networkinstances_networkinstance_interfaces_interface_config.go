...
// Optional[Configuration parameters relating to the associated interface] REF:Optional.empty
type FrinxOpenconfigNetworkInstanceNetworkinstancetopNetworkinstancesNetworkinstanceInterfacesInterfaceConfig struct {
	// Optional[A unique identifier for this interface - this is expressed as a free-text string] REF:Optional.empty
	Id string `json:"id,omitempty"`
	// Optional[Reference to a subinterface -- this requires the base interface to be specified using the interface leaf in this container.  If only a reference to a base interface is requuired, this leaf should not be set.] REF:Optional.empty
	Subinterface int64 `json:"subinterface,omitempty"`
	// Optional[Reference to a base interface.  If a reference to a subinterface is required, this leaf must be specified to indicate the base interface.] REF:Optional.empty
	Interface_ string `json:"interface,omitempty"`
	// Optional[The address families on the subinterface which are to be associated with this network instance. When this leaf-list is empty and the network instance requires Layer 3 information the address families for which the network instance is enabled should be imported. If the value of this leaf-list is specified then the association MUST only be made for those address families that are included in the list.] REF:Optional.empty
	AssociatedAddressFamilies []FrinxOpenconfigNetworkInstanceAssociatedAddressFamiliesIdentityref `json:"associated-address-families,omitempty"`
}
