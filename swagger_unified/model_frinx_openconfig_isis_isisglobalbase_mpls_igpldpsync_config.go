...
// Optional[This container defines ISIS/IGP configuration.] REF:Optional.empty
type FrinxOpenconfigIsisIsisglobalbaseMplsIgpldpsyncConfig struct {
	// Optional[Specifies a delay, expressed in units of seconds, between the LDP session to the IGP neighbor being established, and it being considered synchronized by the IGP.] REF:Optional.empty
	PostSessionUpDelay int32 `json:"post-session-up-delay,omitempty"`
	// Optional[When set to true, rely on IGP/LDP synchronization. IGP cost for link is maintained at max until LDP adjacencies are established ] REF:Optional[RFC5443: LDP IGP Synchronization.]
