...
// Optional[Configurable items for admin-groups] REF:Optional.empty
type FrinxOpenconfigMplsMplsadmingroupstopMplsadmingroupsAdmingroupConfig struct {
	// Optional[bit-position value for mpls admin-group. The value for the admin group is an integer that represents one of the bit positions in the admin-group bitmask. Values between 0 and 31 are interpreted as the original limit of 32 admin groups. Values >=32 are interpreted as extended admin group values as per RFC7308.] REF:Optional.empty
	BitPosition int64 `json:"bit-position,omitempty"`
	// Optional[name for mpls admin-group] REF:Optional.empty
	AdminGroupName string `json:"admin-group-name,omitempty"`
}
