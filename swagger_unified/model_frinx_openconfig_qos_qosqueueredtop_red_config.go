...
// Optional[Configuration data for RED queues] REF:Optional.empty
type FrinxOpenconfigQosQosqueueredtopRedConfig struct {
	// Optional[The maximum threshold parameter for a RED-managed queue. When the average queue length exceeds the maxth value, all packets are dropped (or marked if ECN is enabled).] REF:Optional.empty
	Maxth int32 `json:"maxth,omitempty"`
	// Optional[The mininum threshold parameter for a RED-managed queue. When the average queue length is less than minth, all packets are admitted to the queue.] REF:Optional.empty
	Minth int32 `json:"minth,omitempty"`
}
