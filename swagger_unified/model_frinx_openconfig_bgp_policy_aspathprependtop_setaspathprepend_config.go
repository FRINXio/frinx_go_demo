...
// Optional[Configuration data for the AS path prepend action] REF:Optional.empty
type FrinxOpenconfigBgpPolicyAspathprependtopSetaspathprependConfig struct {
	// Optional[The AS number to prepend to the AS path. If this leaf is not specified and repeat-n is set, then the local AS number will be used for prepending.] REF:Optional.empty
	Asn int64 `json:"asn,omitempty"`
	// Optional[Number of times to prepend the value specified in the asn leaf to the AS path. If no value is specified by the asn leaf, the local AS number of the system is used. The value should be between 1 and the maximum supported by the implementation.] REF:Optional.empty
	RepeatN int32 `json:"repeat-n,omitempty"`
}
