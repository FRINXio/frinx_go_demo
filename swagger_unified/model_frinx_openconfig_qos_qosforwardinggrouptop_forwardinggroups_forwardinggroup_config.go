...
// Optional[Configuration data for forwarding groups] REF:Optional.empty
type FrinxOpenconfigQosQosforwardinggrouptopForwardinggroupsForwardinggroupConfig struct {
	// Optional[Name of the forwarding group] REF:Optional.empty
	Name string `json:"name,omitempty"`
	// Optional[Set the priority for the forwarding group for local transmission through the device, e.g., across a switching fabric. Higher priorities are considered to be better, such that traffic with fabric priority 128 is considered to be higher priority than that with fabric priority 0.] REF:Optional.empty
	FabricPriority int32 `json:"fabric-priority,omitempty"`
	// Optional[Queue for packets in this forwarding group.] REF:Optional.empty
	OutputQueue string `json:"output-queue,omitempty"`
}
