...
// Optional[Configuration parameters enabling or modifying the behavior or enhanced error handling mechanisms for the BGP neighbor] REF:Optional.empty
type FrinxOpenconfigBgpBgpneighborbaseErrorhandlingConfig struct {
	// Optional[Specify whether erroneous UPDATE messages for which the NLRI can be extracted are reated as though the NLRI is withdrawn - avoiding session reset] REF:Optional[draft-ietf-idr-error-handling-16]
	TreatAsWithdraw bool `json:"treat-as-withdraw,omitempty"`
}
