...
// Optional[List of client channels, keyed by index within a physical client port.  A physical port with a single channel would have a single zero-indexed element] REF:Optional.empty
type FrinxOpenconfigPlatformTransceiverPhysicalchanneltopPhysicalchannelsChannel struct {
	// Optional[Reference to the index number of the channel] REF:Optional.empty
	Index int32 `json:"index,omitempty"`
	// Optional[Configuration data for physical channels] REF:Optional.empty
	Config *FrinxOpenconfigPlatformTransceiverPhysicalchanneltopPhysicalchannelsChannelConfig `json:"config,omitempty"`
}
