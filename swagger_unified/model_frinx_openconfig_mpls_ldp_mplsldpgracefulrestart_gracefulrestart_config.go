...
// Optional[LDP graceful-restart configuration attributes.] REF:Optional.empty
type FrinxOpenconfigMplsLdpMplsldpgracefulrestartGracefulrestartConfig struct {
	// Optional[Interval for which the remote LDP peers will wait for the local node to reconnect after a failure] REF:Optional[RFC3478 Graceful Restart Mechanism for Label Distribution Protocol]
	ReconnectTime int32 `json:"reconnect-time,omitempty"`
	// Optional[Enables the graceful restart helper for LDP.] REF:Optional.empty
	...
	// Optional[Interval used to specify the time for the remote peer to maintain the MPLS forwarding state after the local node has succesfully reconnected] REF:Optional[RFC3478 Graceful Restart Mechanism for Label Distribution Protocol]
	RecoveryTime int32 `json:"recovery-time,omitempty"`
	// Optional[Time that defines the interval for keeping the node in recovery mode.] REF:Optional[RFC3478 Graceful Restart Mechanism for Label Distribution Protocol]
	ForwardingHoldtime int32 `json:"forwarding-holdtime,omitempty"`
}
