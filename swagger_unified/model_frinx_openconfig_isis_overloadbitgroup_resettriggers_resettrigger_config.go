...
// Optional[This container defines ISIS Overload Bit reset trigger configuration.] REF:Optional.empty
type FrinxOpenconfigIsisOverloadbitgroupResettriggersResettriggerConfig struct {
	// Optional[In the case that the system sets the overload bit on start, the system should reset the bit (i.e., clear the overload bit) upon the specified trigger.] REF:Optional.empty
	ResetTrigger *FrinxOpenconfigIsisResetTriggerIdentityref `json:"reset-trigger,omitempty"`
	// Optional[If a reset trigger is specified, the system should delay resetting the overload bit for the specified number of seconds after the trigger occurs.] REF:Optional.empty
