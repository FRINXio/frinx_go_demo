...
// Optional[Configuraton parameters relating to route reflection for the BGPgroup] REF:Optional.empty
type FrinxOpenconfigBgpBgpcommonstructureneighborgrouproutereflectorRoutereflectorConfig struct {
	// Optional[Configure the neighbor as a route reflector client.] REF:Optional.empty
	RouteReflectorClient bool `json:"route-reflector-client,omitempty"`
	// Optional[route-reflector cluster id to use when local router is configured as a route reflector.  Commonly set at the group level, but allows a different cluster id to be set for each neighbor.] REF:Optional.empty
	RouteReflectorClusterId string `json:"route-reflector-cluster-id,omitempty"`
}
