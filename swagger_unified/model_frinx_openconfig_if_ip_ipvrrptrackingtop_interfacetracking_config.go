...
// Optional[Configuration data for VRRP interface tracking] REF:Optional.empty
type FrinxOpenconfigIfIpIpvrrptrackingtopInterfacetrackingConfig struct {
	// Optional[Set the value to subtract from priority when the tracked interface goes down] REF:Optional.empty
	PriorityDecrement int32 `json:"priority-decrement,omitempty"`
	// Optional[Sets an interface that should be tracked for up/down events to dynamically change the priority state of the VRRP group, and potentially change the mastership if the tracked interface going down lowers the priority sufficiently] REF:Optional.empty
	TrackInterface string `json:"track-interface,omitempty"`
}
