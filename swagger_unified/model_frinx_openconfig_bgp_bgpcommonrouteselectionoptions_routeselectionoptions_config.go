...
// Optional[Configuration parameters relating to route selection options] REF:Optional.empty
type FrinxOpenconfigBgpBgpcommonrouteselectionoptionsRouteselectionoptionsConfig struct {
	// Optional[Compare multi-exit discriminator (MED) value from different ASes when selecting the best route.  The default behavior is to only compare MEDs for paths received from the same AS.] REF:Optional.empty
	AlwaysCompareMed bool `json:"always-compare-med,omitempty"`
	// Optional[Advertise inactive routes to external peers.  The default is to only advertise active routes.] REF:Optional.empty
	AdvertiseInactiveRoutes bool `json:"advertise-inactive-routes,omitempty"`
	// Optional[Ignore the AS path length when selecting the best path. The default is to use the AS path length and prefer paths with shorter length.] REF:Optional.empty
	IgnoreAsPathLength bool `json:"ignore-as-path-length,omitempty"`
	// Optional[Flag to enable sending / receiving accumulated IGP attribute in routing updates] REF:Optional.empty
	EnableAigp bool `json:"enable-aigp,omitempty"`
	// Optional[Ignore the IGP metric to the next-hop when calculating BGP best-path. The default is to select the route for which the metric to the next-hop is lowest] REF:Optional.empty
	IgnoreNextHopIgpMetric bool `json:"ignore-next-hop-igp-metric,omitempty"`
	// Optional[When comparing similar routes received from external BGP peers, use the router-id as a criterion to select the active path.] REF:Optional.empty
	ExternalCompareRouterId bool `json:"external-compare-router-id,omitempty"`
}
