...
// Optional[The list of configured IPv4 addresses on the interface.] REF:Optional.empty
type FrinxOpenconfigIfIpIpv4topIpv4AddressesAddress struct {
	// Optional[Configuration data for each configured IPv4 address on the interface] REF:Optional.empty
	Config *FrinxOpenconfigIfIpIpv4topIpv4AddressesAddressConfig `json:"config,omitempty"`
	// Optional[References the configured IP address] REF:Optional.empty
	Ip string `json:"ip,omitempty"`
	// Optional[Enclosing container for VRRP groups handled by this IP interface] REF:Optional[RFC 5798 - Virtual Router Redundancy Protocol (VRRP) Version 3 for IPv4 and IPv6]
	FrinxOpenconfigIfIpvrrp *FrinxOpenconfigIfIpIpvrrptopVrrp `json:"frinx-openconfig-if-ip:vrrp,omitempty"`
}
