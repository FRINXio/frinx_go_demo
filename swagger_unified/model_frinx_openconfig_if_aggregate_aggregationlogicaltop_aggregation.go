...
type FrinxOpenconfigIfAggregateAggregationlogicaltopAggregation struct {
	// Optional[Enclosing container for VLAN interface-specific data on Ethernet interfaces.  These are for standard L2, switched-style VLANs.] REF:Optional.empty
	FrinxOpenconfigVlanswitchedVlan *FrinxOpenconfigVlanVlanswitchedtopSwitchedVlan `json:"frinx-openconfig-vlan:switched-vlan,omitempty"`
	// Optional[Enclosing container for BFD interface-specific data.] REF:Optional.empty
	FrinxBfdbfd *FrinxBfdBfdtopBfd `json:"frinx-bfd:bfd,omitempty"`
	// Optional[Configuration variables for logical aggregate / LAG interfaces] REF:Optional.empty
	Config *FrinxOpenconfigIfAggregateAggregationlogicaltopAggregationConfig `json:"config,omitempty"`
}
