...
// Optional[Configuration data for port breakout] REF:Optional.empty
type FrinxOpenconfigPlatformPortPortbreakouttopBreakoutmodeConfig struct {
	// Optional[Sets the number of channels to 'breakout' on a port capable of channelization] REF:Optional.empty
	NumChannels int32 `json:"num-channels,omitempty"`
	// Optional[Sets the channel speed on each channel -- the supported values are defined by the ETHERNET_SPEED identity] REF:Optional.empty
	ChannelSpeed *FrinxOpenconfigPlatformPortChannelSpeedIdentityref `json:"channel-speed,omitempty"`
}
