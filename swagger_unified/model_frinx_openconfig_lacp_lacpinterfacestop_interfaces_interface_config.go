...
// Optional[Configuration data for each LACP aggregate interface] REF:Optional.empty
type FrinxOpenconfigLacpLacpinterfacestopInterfacesInterfaceConfig struct {
	...
	// Optional[Reference to the interface on which LACP should be configured.   The type of the target interface must be ieee8023adLag] REF:Optional.empty
	Name string `json:"name,omitempty"`
	// Optional[Set the period between LACP messages -- uses the lacp-period-type enumeration.] REF:Optional.empty
	Interval *FrinxOpenconfigLacpLacpPeriodType `json:"interval,omitempty"`
	// Optional[Indicates participant is active or passive] REF:Optional.empty
	LacpMode *FrinxOpenconfigLacpLacpActivityType `json:"lacp-mode,omitempty"`
	// Optional[Sytem priority used by the node on this LAG interface. Lower value is higher priority for determining which node is the controlling system.] REF:Optional.empty
