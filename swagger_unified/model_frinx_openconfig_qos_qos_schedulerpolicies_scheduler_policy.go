...
// Optional[List of scheduler policies. A scheduler policy is a set of schedulers that are to be applied together. Each scheduler within a scheduler policy takes an input, and outputs it according to a scheduling discipline that is specified within it. The schedulers consume resources according to the specification that is provided - which may be absolute resource limits, or relative.] REF:Optional.empty
type FrinxOpenconfigQosQosSchedulerpoliciesSchedulerPolicy struct {
	// Optional[Reference to the name of the scheduler policy] REF:Optional.empty
	Name string `json:"name,omitempty"`
	// Optional[Schedulers within the scheduler policy.] REF:Optional.empty
	Schedulers *FrinxOpenconfigQosQosSchedulerpoliciesSchedulerpolicySchedulers `json:"schedulers,omitempty"`
	// Optional[Configuration parameters relating to a scheduler policy.] REF:Optional.empty
	Config *FrinxOpenconfigQosQosschedulertopSchedulerpoliciesSchedulerpolicyConfig `json:"config,omitempty"`
}
