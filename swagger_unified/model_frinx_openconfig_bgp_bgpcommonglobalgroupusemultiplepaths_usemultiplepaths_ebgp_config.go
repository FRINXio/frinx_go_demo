...
// Optional[Configuration parameters relating to eBGP multipath] REF:Optional.empty
type FrinxOpenconfigBgpBgpcommonglobalgroupusemultiplepathsUsemultiplepathsEbgpConfig struct {
	// Optional[Allow multipath to use paths from different neighbouring ASes.  The default is to only consider multiple paths from the same neighbouring AS.] REF:Optional.empty
	AllowMultipleAs bool `json:"allow-multiple-as,omitempty"`
	// Optional[Maximum number of parallel paths to consider when using BGP multipath. The default is use a single path.] REF:Optional.empty
	MaximumPaths int64 `json:"maximum-paths,omitempty"`
}
