...
// Optional[Configuration parameters relating to RSVP soft preemption support] REF:Optional.empty
type FrinxOpenconfigMplsRsvpMplsrsvpsoftpreemptionSoftpreemptionConfig struct {
	// Optional[Timeout value for soft preemption to revert to hard preemption. The default timeout for soft-preemption is 30 seconds - after which the local system reverts to hard pre-emption.] REF:Optional[RFC5712 MPLS-TE soft preemption]
	SoftPreemptionTimeout int32 `json:"soft-preemption-timeout,omitempty"`
	// Optional[Enables soft preemption on a node.] REF:Optional.empty
