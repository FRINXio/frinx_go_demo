...
// Optional[Configuration data for aggregate advertisements] REF:Optional.empty
type FrinxOpenconfigNetworkInstanceNetworkinstancesNetworkinstanceProtocolsProtocolLocalaggregatesAggregateConfig struct {
	// Optional[Set a generic tag value on the route. This tag can be used for filtering routes that are distributed to other routing protocols.] REF:Optional.empty
	SetTag string `json:"set-tag,omitempty"`
	// Optional[When true, install the aggregate route with a discard next-hop -- traffic destined to the aggregate will be discarded with no ICMP message generated.  When false, traffic destined to an aggregate address when no constituent routes are present will generate an ICMP unreachable message.] REF:Optional.empty
	Discard bool `json:"discard,omitempty"`
	// Optional.empty REF:Optional.empty
	FrinxBgpExtensionsummaryOnly bool `json:"frinx-bgp-extension:summary-only,omitempty"`
	// Optional[Name of policy which needs to be applied.] REF:Optional.empty
	FrinxBgpExtensionapplyPolicy []string `json:"frinx-bgp-extension:apply-policy,omitempty"`
	// Optional[Aggregate prefix to be advertised] REF:Optional.empty
