...
// Optional[A range of labels starting with the start-label up to and including the end label that should be allocated for use by a specific protocol.] REF:Optional.empty
type FrinxOpenconfigMplsMplslabelblockstopReservedlabelblocksReservedLabelBlock struct {
	// Optional[Configuration parameters relating to the label block.] REF:Optional.empty
	Config *FrinxOpenconfigMplsMplslabelblockstopReservedlabelblocksReservedlabelblockConfig `json:"config,omitempty"`
	// Optional[A reference to a unique local identifier for this label block.] REF:Optional.empty
