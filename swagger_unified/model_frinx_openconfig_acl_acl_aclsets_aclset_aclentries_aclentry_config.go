...
type FrinxOpenconfigAclAclAclsetsAclsetAclentriesAclentryConfig struct {
	// Optional[A user-defined description, or comment, for this Access List Entry.] REF:Optional.empty
	Description string `json:"description,omitempty"`
	// Optional[terminal name] REF:Optional.empty
	FrinxAclExtensiontermName string `json:"frinx-acl-extension:term-name,omitempty"`
	// Optional[The sequence id determines the order in which ACL entries are applied.  The sequence id must be unique for each entry in an ACL set.  Target devices should apply the ACL entry rules in the order determined by sequence id, rather than the relying only on order in the list.] REF:Optional.empty
