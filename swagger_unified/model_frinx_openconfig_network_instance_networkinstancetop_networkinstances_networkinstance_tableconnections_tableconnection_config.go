...
// Optional[Configuration parameters relating to the connection between tables] REF:Optional.empty
type FrinxOpenconfigNetworkInstanceNetworkinstancetopNetworkinstancesNetworkinstanceTableconnectionsTableconnectionConfig struct {
	// Optional[explicitly set a default policy if no policy definition in the import policy chain is satisfied.] REF:Optional.empty
	DefaultImportPolicy *FrinxOpenconfigRoutingPolicyDefaultPolicyType `json:"default-import-policy,omitempty"`
	// Optional[The destination protocol for the table connection] REF:Optional.empty
	DstProtocol *FrinxOpenconfigNetworkInstanceDstProtocolIdentityref `json:"dst-protocol,omitempty"`
	// Optional[The source protocol for the table connection] REF:Optional.empty
	SrcProtocol *FrinxOpenconfigNetworkInstanceSrcProtocolIdentityref `json:"src-protocol,omitempty"`
	// Optional[list of policy names in sequence to be applied on receiving a routing update in the current context, e.g., for the current peer group, neighbor, address family, etc.] REF:Optional.empty
	ImportPolicy []string `json:"import-policy,omitempty"`
	// Optional[The address family associated with the connection. This must be defined for the source protocol. The target address family is implicitly defined by the address family specified for the source protocol.] REF:Optional.empty
	AddressFamily *FrinxOpenconfigNetworkInstanceAddressFamilyIdentityref `json:"address-family,omitempty"`
}
