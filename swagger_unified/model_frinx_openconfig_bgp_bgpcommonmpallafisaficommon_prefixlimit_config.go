...
// Optional[Configuration parameters relating to the prefix limit for the AFI-SAFI] REF:Optional.empty
type FrinxOpenconfigBgpBgpcommonmpallafisaficommonPrefixlimitConfig struct {
	// Optional[Time interval in seconds after which the BGP session is re-established after being torn down due to exceeding the max-prefix limit.] REF:Optional.empty
	RestartTimer int32 `json:"restart-timer,omitempty"`
	// Optional[Threshold on number of prefixes that can be received from a neighbour before generation of warning messages or log entries. Expressed as a percentage of max-prefixes] REF:Optional.empty
	ShutdownThresholdPct int32 `json:"shutdown-threshold-pct,omitempty"`
	// Optional[Maximum number of prefixes that will be accepted from the neighbour] REF:Optional.empty
	MaxPrefixes int64 `json:"max-prefixes,omitempty"`
	// Optional[Do not tear down the BGP session when the maximum prefix limit is exceeded, but rather only log a warning. The default of this leaf is false, such that when it is not specified, the session is torn down.] REF:Optional.empty
	PreventTeardown bool `json:"prevent-teardown,omitempty"`
}
