...
// Optional[This container defines ISIS graceful-restart configuration.] REF:Optional.empty
type FrinxOpenconfigIsisIsisglobalbaseGracefulrestartConfig struct {
	...
	// Optional[Enable or disable the IS-IS graceful restart helper function. When this leaf is set, the local system does not utilise the IS-IS graceful restart procedures during its own restart, but supports retaining forwarding information during a remote speaker's restart.] REF:Optional.empty
