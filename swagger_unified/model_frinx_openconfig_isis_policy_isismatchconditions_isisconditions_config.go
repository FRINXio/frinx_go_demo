...
// Optional[Configuration parameters relating to IS-IS match conditions] REF:Optional.empty
type FrinxOpenconfigIsisPolicyIsismatchconditionsIsisconditionsConfig struct {
	// Optional[Match the level that the IS-IS prefix is within. This can be used in the case that import or export policies refer to an IS-IS instance that has multiple levels configured within it] REF:Optional.empty
	LevelEq int32 `json:"level-eq,omitempty"`
}
