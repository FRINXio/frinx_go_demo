...
// Optional[A list of connections between pairs of routing or forwarding tables, the leaking of entries between which is specified by the import policy.  A connection connecting a source table to a destination table implies that routes that match the policy specified for the connection are available for the destination protocol to advertise, or match within its policies.] REF:Optional.empty
type FrinxOpenconfigNetworkInstanceNetworkinstancetopNetworkinstancesNetworkinstanceTableconnectionsTableConnection struct {
	// Optional[The table to which routing entries should be exported] REF:Optional.empty
	DstProtocol string `json:"dst-protocol,omitempty"`
	// Optional[The name of the protocol associated with the table which should be utilised as the source of forwarding or routing information] REF:Optional.empty
	SrcProtocol string `json:"src-protocol,omitempty"`
	// Optional[Configuration parameters relating to the connection between tables] REF:Optional.empty
	Config *FrinxOpenconfigNetworkInstanceNetworkinstancetopNetworkinstancesNetworkinstanceTableconnectionsTableconnectionConfig `json:"config,omitempty"`
	// Optional[The address family associated with the connection] REF:Optional.empty
