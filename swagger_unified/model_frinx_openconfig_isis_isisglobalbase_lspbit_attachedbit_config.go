...
// Optional[This container defines Attached Bit configuration.] REF:Optional.empty
type FrinxOpenconfigIsisIsisglobalbaseLspbitAttachedbitConfig struct {
	// Optional[When set to true, if the attached bit is set on an incoming Level 1 IS-IS, the local system ignores it. In this case the local system does not set a default route to the L1L2 router advertising the PDU with the attached bit set.] REF:Optional.empty
	IgnoreBit bool `json:"ignore-bit,omitempty"`
	// Optional[When set to true, if the local IS acts as a L1L2 router, then the attached bit is not advertised in locally generated PDUs.] REF:Optional.empty
	SuppressBit bool `json:"suppress-bit,omitempty"`
}
